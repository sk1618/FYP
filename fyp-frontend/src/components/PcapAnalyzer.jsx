...
export default function PcapAnalyzer() {
  ...
  const handleAnalyze = async () => {
    if (!file) { setError("Please select a .pcap file first."); return; }
    const formData = new FormData();
    formData.append("pcap", file);
    setLoading(true); setLiveLoading(false); setError(null); setResult(null); setShowAllEvents(false);
    try {
  ...
  const handleLiveScan = async () => {
    setLiveLoading(true); setLoading(false); setError(null); setResult(null); setShowAllEvents(false);
    try {
      const res = await axios.post(`${API_BASE}/api/ai/live-scan`, { duration, iface: "any" });
      setResult(res.data);
    } catch (err) {
  ...
  const visibleEvents    = showAllEvents ? report : report.slice(0, EVENTS_PAGE_SIZE);
  ...
  return (
    <div className="panel">
      ...
      <div style={{ display: "flex", gap: "0.75rem", alignItems: "center", flexWrap: "wrap" }}>
        ...
        <button className="btn btn--primary" onClick={handleAnalyze} disabled={loading || liveLoading || !file}>
      ...
      <div style={{ display: "flex", gap: "0.75rem", alignItems: "center", marginTop: "0.75rem", flexWrap: "wrap" }}>
        <select
          ...
          style={{ padding: "0.55rem 0.8rem", borderRadius: "10px", border: "1px solid rgba(255,255,255,0.15)", background: "#111827", color: "white" }}
        >
        ...
        <span style={{ fontSize: "0.78rem", color: "rgba(255,255,255,0.45)", fontStyle: "italic", letterSpacing: "0.01em" }}>
      ...
      {result && (
        <div className="pcap-result">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "1rem", marginTop: "0.75rem" }}>
                {Object.entries(trainingMetrics).map(([modelName, values]) => (
                  <div key={modelName} style={{
                    ...
                    <div style={{ fontWeight: "700", marginBottom: "0.75rem", color: "#e2e8f0", fontSize: "0.95rem" }}>
                      {modelName}
                    </div>
          ...
          {/* ── 5. Attack summary (raw counts) ────────────────────────── */}
          <div className="pcap-result-section">
            <div className="pcap-section-title">Attack Summary — Random Forest</div>
            <pre className="pcap-pre">{JSON.stringify(result.summary, null, 2)}</pre>
          </div>

          {result.lr_summary && Object.keys(result.lr_summary).length > 0 && (
            <div className="pcap-result-section">
              <div className="pcap-section-title">Attack Summary — Logistic Regression</div>
              <pre className="pcap-pre">{JSON.stringify(result.lr_summary, null, 2)}</pre>
            </div>
          )}

          {/* ── 6. Detected events (collapsible) ──────────────────────── */}
          {report.length > 0 && (
            <div className="pcap-result-section">
              <div className="pcap-section-title">
                Detected Events — Random Forest ({report.length})
              </div>
              ...
              {report.length > EVENTS_PAGE_SIZE && (
                <button
                ...
                >
                  {showAllEvents
                    ? "Show less"
                    : `Show ${report.length - EVENTS_PAGE_SIZE} more…`}
                </button>
              )}
            </div>
          )}
